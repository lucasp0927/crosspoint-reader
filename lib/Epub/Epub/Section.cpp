...
// v39: vertical-mode image page breaks are decided on the block axis, so an
...
//      mid-chapter images differ from v38. Horizontal layout is unchanged.
...
constexpr uint32_t HEADER_SIZE = sizeof(uint8_t) + sizeof(int) + sizeof(float) + sizeof(bool) + sizeof(uint8_t) +
                                 ...
                                 sizeof(uint8_t) + sizeof(bool) + sizeof(bool) + sizeof(uint32_t) +
...
void Section::writeSectionFileHeader(const ReaderRenderSpec& spec) {
  ...
  static_assert(HEADER_SIZE == sizeof(SECTION_FILE_VERSION) + sizeof(spec.fontId) + sizeof(spec.lineCompression) +
                                   ...
                                   sizeof(spec.verticalMode) + sizeof(uint32_t) + sizeof(uint32_t) +
  ...
  serialization::writePod(file, spec.verticalMode);
...
bool Section::loadSectionFile(const ReaderRenderSpec& spec) {
  ...
  bool filePartial = false;
  {
    ...
    if (spec.fontId != fileFontId || spec.lineCompression != fileLineCompression ||
        ...
        spec.verticalMode != fileVerticalMode) {
      file.close();
...
bool Section::startBuild(const ReaderRenderSpec& spec, const std::function<void()>& popupFn) {
  ...
  // Vertical mode lays out in transposed space: the inline axis (line length)
  // runs down the screen, the block axis (line stacking) runs right-to-left.
  // The parser works entirely in that space; rendering maps it back (PageLine).
  const uint16_t layoutWidth = spec.verticalMode ? spec.viewportHeight : spec.viewportWidth;
  const uint16_t layoutHeight = spec.verticalMode ? spec.viewportWidth : spec.viewportHeight;
  ctx->parser = makeUniqueNoThrow<ChapterHtmlSlimParser>(
      epub, ctxPtr->parsePath, renderer, spec.fontId, spec.lineCompression, spec.extraParagraphSpacing,
      spec.paragraphAlignment, layoutWidth, layoutHeight, spec.hyphenationEnabled, spec.focusReadingEnabled,
      spec.verticalMode,
